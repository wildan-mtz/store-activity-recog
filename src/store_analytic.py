...
class StoreAnalytic:
    ...
    ### ADD ### Tambah argumen def run ### ADD ###
    def run(self, vis_region=False, source="cam", oFrame_width=614, oFrame_height=614, container=None, table=None):
        """ Run analytic sytem"""
        ...
        while True:
            ...
            img = Image.fromarray(img).resize((int(oFrame_width), int(oFrame_height)))
            ...
            for item in table.get_children():
                table.delete(item) 
            for num, region in enumerate(self.analytic_result):
                person_count = self.analytic_result[region][0]
                time = self.analytic_result[region][1]
                table.insert(parent='', index='end',iid=num,
                                text='', values=(region, person_count, time))
            ### ADD ###
